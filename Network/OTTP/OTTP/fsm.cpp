...
enum pakcet_type
{
	F_CON = 0, F_FIN = 1, F_ACK = 2, F_DATA = 3
};   // Packet Type
enum proto_state
{
	wait_CON = 0, CON_sent = 1, CONNECTED = 2
};     // States
...
struct packet
{               // 504 Byte Packet to & from Simulator
				// enum packet_type
	unsigned short type;        
	unsigned short size;
...
void set_timer(int sec)
{
	...
	liDueTime.QuadPart = -10000000 * sec;
...
loop:
	...
	if(!kbhit())
	{
		...
		else
		{
			...
			size_t n = Recv((char*) &event.packet, sizeof(struct packet));
			if(n > 0)
			{
